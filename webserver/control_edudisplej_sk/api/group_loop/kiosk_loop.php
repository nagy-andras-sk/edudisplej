...
try {
    ...
    if (empty($loops)) {
        $stmt = $conn->prepare("SELECT kgm.*, m.name as module_name, m.module_key, m.description,
                    ...
                    (SELECT klp.updated_at FROM kiosk_group_loop_plans klp WHERE klp.group_id = kga.group_id LIMIT 1) as plan_updated_at,
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            if (!empty($row['plan_updated_at']) && (!$group_updated_at || strtotime($row['plan_updated_at']) > strtotime($group_updated_at))) {
                $group_updated_at = $row['plan_updated_at'];
            }
            $group_plan_version = max((int)$group_plan_version, (int)($row['plan_version'] ?? 0));
        }
        $stmt->close();
    }
    ...
    if (!empty($loops)) {
        ...
        $response['loop_plan_version'] = isset($group_plan_version) ? (int)$group_plan_version : 0;
    }
